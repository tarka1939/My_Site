...
2. **JDK 25 + Maven were not installed locally** (only JRE 8 present, no Maven at all). Installed Temurin 25 JDK via `winget` and Maven 3.9.16 via a manual download from `dlcdn.apache.org` (winget has no Maven package) with SHA-512 checksum verification, after explicit user confirmation for both — downloading files/modifying system PATH are permission-gated actions per the operating rules, so asked before acting rather than assuming.
