...
- **A closing keyword works anywhere in the body, including inside a sentence that denies it** (learned 2026-08-10, the hard way). PR #100 deliberately carried no closing keyword and opened with "Advances #49. **Deliberately does not close it**" — then explained further down that "Applying this to a live site is what closes #49." GitHub matched `closes #49` in that prose and closed the issue on merge. The parser has no notion of context, negation, or the sentence around it: a keyword followed by an issue reference *anywhere* in the body is a closing link. So when writing about what *would* close an issue, or what a different PR will close, avoid the keyword entirely — write "is what completes #49", or name the issue without a verb in front of it. And check `closingIssuesReferences` before merging a PR that is meant *not* to close something, not only one that is.
