...
> Status (2026-09-24): Phases 0-4 complete; Phase 5 (deploy) has both halves live and both deploy pipelines live on `main` — **any push or merge to `main` now deploys to production** — with the first `dev` → `main` promotion through them pending; Phase 6 (content & polish) and Phase 8 (polishing) in progress; Phase 7a (`githubsync`) complete behind a feature flag. `PROJECT_TODO.md` is authoritative — this line is a pointer, not a substitute.
...
- **The remote's name depends on the checkout.** It is `My_Site` in the owner's local clones and `origin` in Claude Code cloud sessions, which clone fresh with git's default; same repository either way. Run `git remote` before copying a command, and substitute. `My_Site/...` below means "this remote". A wrong name fails loudly (`unknown revision`), so the risk is wasted steps, not a silent wrong result. A cloud clone may start with only `dev` and the session branch — `git fetch origin main` before touching `main`.
...
to `http://localhost:8080` so the browser sees same-origin requests — the backend's CORS allowlist
(`app.cors.allowed-origins`) holds only the deployed Netlify origin, never `localhost:4200`, so
...
# up; Playwright starts the backend and `ng serve` itself, or reuses running ones:
cd e2e && npm install && npm run install:browsers   # once
cd e2e && npm test
cd e2e && npx playwright test tests/admin.spec.ts   # one journey
cd e2e && npx playwright test -g "rate limit"       # one test by name

# PreToolUse hook cases (run after touching anything in .claude/hooks/):
bash .claude/hooks/hooks.test.sh

# Content seeding (Node stdlib only, no npm install). Copy in projects.json is not signed off —
# read content-seed/README.md before running it against anything:
node content-seed/seed.mjs --dry-run
...
- **SPA routing:** Netlify handles this natively via a `frontend/public/_redirects` file (`/* /index.html 200`, below one `/about / 301` rule — Netlify applies the first match) — no GitHub-Pages-style `404.html` copy trick needed. `--base-href` uses the Angular default (`/`), since Netlify serves from root rather than a repo-name subpath.
...
- **Backend layering:** package-by-feature, enforced with **Spring Modulith** — not a single layered controller/service/repository split. Feature packages under `io.github.tarka1939.mysite`: `project/`, `contact/`, `auth/`, `about/`, `githubsync/` (Phase 7a); Phase 7 still plans `analytics/`, `agentlog/`, `dspdemo/`. Cross-cutting pieces live in the root package — `SecurityConfig`, `GlobalExceptionHandler`, `InMemoryRateLimiter`, `ClientIpResolver`, `AsyncConfig`, `ResendEmailClient`, shared exceptions. A Spring Modulith verification test (`ApplicationModules.verify()`) enforces that packages only interact through public APIs or events, not internal classes. Within each package, still keep DTOs at the controller boundary — never return JPA entities directly from controllers.
...
- **Local dev CORS:** the CORS allowlist covers only the deployed origin — `ng serve` stays same-origin via a dev-server proxy (`frontend/proxy.conf.json` forwards `/api/*` to `localhost:8080`) rather than adding `localhost` to the allowlist. `environment.development.ts`'s `apiBaseUrl` is deliberately a relative path (`/api/v1`) for this to work; `environment.ts` (prod) stays an absolute URL.
...
- **Trust boundaries.** Don't trust a client-supplied header or value unless something upstream is known to set or sanitize it. In particular, never read `X-Forwarded-For`/`X-Real-IP` directly for IP-based logic — go through `ClientIpResolver` (#168), which honours forwarded headers only from configured `trusted-proxies` at a fixed `trusted-hop-count`, and is off in the base config so dev/test/no-profile trust nothing but `getRemoteAddr()`. A forged header with a fresh value per request defeats per-IP rate limiting outright.
...
- `docs/AUTONOMOUS_WORKFLOW.md` — the operative workflow from Phase 4's tail to the end of the project: one persistent "Senior Dev" session, independent fresh-session PR review, ambiguity/escalation handling, and the Phase 5 pre-flight checklist
