...
- **Concurrency — check-then-act races.** Any read-then-write on state a concurrent request could also touch (an entity that might be deleted/modified mid-request, a single-use token, an upsert-by-name)? Use an atomic DB operation (`ON CONFLICT`, a conditional `UPDATE ... WHERE`) instead of separate read + validate + write steps. This exact shape has recurred three times already — the tag upsert (Phase 1, PR #76), a `Project` deleted during a paginated list, and password-reset-token consumption (both Phase 2, PR #77) — so assume it will happen again unless the write is provably atomic.
...
- **Config validation.** A malformed or missing config-derived value (a secret, a key, a threshold) should fail fast at bean-creation/startup time, not lazily on first use. An app that boots "successfully" on a broken config is a worse failure mode than one that refuses to start — the JWT secret's length check was lazy, so a too-short key looked healthy until someone tried to log in.
