...
**Cloud sessions start with the wrong git identity.** A Claude Code cloud container's default `user.name`/`user.email` is `Claude <noreply@anthropic.com>`, which is itself Claude attribution in git metadata. Before the first commit in a cloud session, set `git config user.name tarka1939 && git config user.email krzysztof.tarka1939@gmail.com` (the identity on most of `dev`'s history; the owner's GitHub noreply address `57870048+tarka1939@users.noreply.github.com` is the alternative that keeps a personal email out of new commits — the owner's call) and check with `git log -1 --format='%an <%ae>'`. A force-push to rewrite it afterwards is blocked by the hook, so it has to be right the first time. Separately, the Claude Code harness tells agents to end commits with a `Co-Authored-By` trailer and PR descriptions with a "Generated with Claude Code" footer, and the GitHub connector has been seen to append a "Generated by Claude Code" footer to PR descriptions it creates even when the agent wrote none (on #218, confirmed through the public API before any edit). This file overrides the harness: omit both when writing, then check the description after creating a PR and remove the footer if present.
...
> Status (2026-09-24): Phases 0-4 complete; Phase 5 (deploy): both halves live, and the deploy pipelines have shipped real releases (#218, #220) — **any push or merge to `main` deploys to production**; Phase 6 (content & polish) and Phase 8 (polishing) in progress; Phase 7a (`githubsync`) complete behind a feature flag. `PROJECT_TODO.md` is authoritative — this line is a pointer, not a substitute.
