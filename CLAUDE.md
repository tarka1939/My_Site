...
Run the regenerate and require **`git diff --numstat`** to come back empty — not `git status --porcelain`, which misreads this on Windows. `.gitattributes` is `* text=auto eol=lf` while `core.autocrlf` is `true`, so the generator's CRLF output makes `.openapi-generator/FILES` show as modified when its content is byte-identical after normalisation. That looks exactly like the PR #129 stale-client bug and is not it. When starting contract
