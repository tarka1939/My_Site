...
From Phase 4's tail to the end of the project (extended from "through Phase 6" on 2026-09-24), the project runs on the model in `docs/AUTONOMOUS_WORKFLOW.md` rather than a hand-written kickoff prompt per phase — see that doc and the corresponding ADR in `docs/DECISIONS.md` for the full reasoning. Summary:
...
- Every PR gets reviewed by a fresh, independent Claude Code session with no shared context with the implementation, dispatched by the Senior Dev into its own detached worktree. GitHub Copilot's review is **optional, not a merge gate** (since 2026-09-24 — its quota runs out within days at this project's PR volume; see `docs/AUTONOMOUS_WORKFLOW.md`). The independent review is mandatory and has no fallback.
...
> **Status (2026-09-25): complete, except #41, which is deferred on purpose.** Both halves deploy themselves on every merge to `main` and have done so for four releases (#218, #220, #226, #229). The rollback is proven against a build that compiles and refuses to start. Its cost is about two minutes of downtime per bad release while the 90s health check waits, so it is not zero-downtime. #38, #45 and #46 and both epics (#74, #75) were closed on 2026-09-25, each with the run ids and log lines that meet its acceptance. One criterion is inferred rather than observed: "exactly one Netlify deploy per merge". The production deploy is CLI-sourced and was not replaced by a native build, but the Netlify connector cannot list deploys; #38's closing comment says how to confirm it. Remaining in the milestone: #181 (the runbook still says a backend host change is one edit; it is four) and #41. `docs/CI_PLAN.md` §8 and `docs/DEPLOY_PIPELINE_SETUP.md` are the record.
>
> **Superseded (2026-09-24):** **both halves live; CI exists; both deploy pipelines are live on `main`, their rollback is proven, and they have shipped real releases (#218, 87 commits including a migration; #220) — any push or merge to `main` now deploys.** Open in this milestone: #38, #45 and #46 describe exactly what those releases exercised and can likely be closed (owner's call); #41 is deferred on purpose; #181 and #203 are smaller follow-ups; and the epics #74/#75 can be closed once their children are (#75 contains the deferred #41). *(Earlier status, 2026-09-05: the deploy automation was written but not switched on.)* The backend answers on `https://tarka1939.bieda.it`, Netlify serves the promoted frontend against it, TLS verifies clean on both (#47), the SPA fallback is confirmed on the deployed artifact (#39), Postgres is verified by its own migrations (#43), and the admin password is set (#121). **CI now runs both suites plus an API-client-staleness check on every pull request** (#193) — and caught a racy test on its first day that passes locally and failed on the runner. Structured logging landed off-by-default (#48). As of 2026-09-05, what remained was six items: the two deploy pipelines were *written* (#38, #45, PR #196) and waited on credentials and a Netlify switch that only the owner could do; #46 was those credentials; #42 was independent; #41 was deferred on purpose. (As of 2026-09-24 the credentials and the switch are done and both pipelines have run; #42 landed; what was left was the first promotion through the pipelines — shipped the same evening, #218 — and #41, still deferred.) Order and acceptance criteria in `docs/CI_PLAN.md`, setup steps in `docs/DEPLOY_PIPELINE_SETUP.md`, decisions in `docs/DECISIONS.md` 2026-09-04. **"Phase 5 is nearly done" is still the wrong reading**: the manual half is finished, which is the half that tells you what to automate. **Update (2026-09-24):** the deploy workflow files are now on `main` too (PR #215, a cherry-pick of #196's commit merged with `[skip ci]`, so nothing deployed), which is what makes `docs/CI_PLAN.md` §8 step 2's dispatch-against-`main` possible. Later the same day: owner setup done, both pipelines dispatched green against `main` (the backend's `command=` restriction and host-key pin exercised for real), and the rollback proven against a build that compiles and refuses to start under `prod` — site back in ~2m17s, run red, failed jar kept. The promotion followed that evening (#218) — the pipeline's first *change* rather than its first run — with no rollback.
>
...
- [x] Deploy the frontend from Actions, replacing Netlify's own build — done (#38, PR #196; live
  2026-09-24). Builds, asserts `_redirects` reached the artifact, publishes via the Netlify CLI, then
  checks a deep link returns 200. Netlify's git build is off, which is what makes it a move rather than
  a second build racing the first; see #38's closing comment for how that was established
- [x] Confirm the `_redirects` SPA fallback survives the build — verified on the **deployed** artifact (`/projects/does-not-exist` returns 200, not 404). First confirmed on Netlify's own build (there was no CI then); since 2026-09-24 the Actions build is what ships, and it asserts `_redirects` survived before publishing
- [x] Custom domain — decided: not now (#40, closed). If one is added later, add it to `app.cors.allowed-origins` and to the canonical origin in `index.html`, `robots.txt` and `sitemap.xml` (#182) in the same change
...
- [x] `docker-compose.yml` for local dev — done (#42, PR #201, 2026-09-06). Postgres only, not
  the backend (there is no image to run it from while #41 is deferred), on `postgres:16-alpine` to
  match production and the Testcontainers suite (#200). `docker compose up -d --wait` replaces the
  old "point the `DB_*` variables at whatever Postgres you have" paragraph
...
- [x] Deploy the backend jar to the VPS on merge to `main` — done (#45, PR #196; live 2026-09-24).
  It deploys the **jar**, not an image (#41 deferred). The jar arrives on stdin because the SSH key is
  restricted with `command=` to `deploy/deploy.sh` and can run nothing else. The script ignores the
  client-supplied command, swaps the jar with the previous one kept, restarts through a scoped
  `NOPASSWD` sudoers line, and rolls back if the health check fails. The rollback was proven in run
  36050901571
- [x] Secrets via CI/CD secret store — done (#46). Six values in the Actions store, named in
  `docs/DEPLOY_PIPELINE_SETUP.md` along with what each could do if it leaked. Checked on 2026-09-25:
  the latest public deploy logs contain none of them and no host details, only GitHub's `***` mask.
  Actions is a **second** store; `/etc/mysite/env` stays the application's own
...
- [x] **Not in the original plan** — the contact form notified nobody, issue #186. `ContactService.submit` persisted a `ContactMessage` and stopped there, so a visitor was told *"I'll get back to you soon"* and the only thing making that true was the owner remembering to open `/admin/messages`. Never scoped rather than regressed: `SPEC.md` promises "Contact form, with basic rate limiting" and the visitor-side story was satisfied. Fixed with a `ContactMessageReceivedEvent` and an `@Async` `AFTER_COMMIT` listener that emails the owner and **swallows its own failures** — notification is best-effort, persistence is not, and `AGENT_LOG.md` records the opposite shape shipping once already in `PasswordResetService.requestReset`. New `CONTACT_NOTIFICATION_EMAIL` variable: absent degrades, malformed refuses to start. Consequence worth knowing: **`RESEND_API_KEY` is no longer demo-only** — it now gates an operational need, not just the password-reset showcase. A cold review then returned DO NOT MERGE on four findings, all applied: the `@Async` dispatch could be rejected before the listener's own `catch` existed (no rejection handler on `taskExecutor`, so `AbortPolicy` applied); `ResendEmailClient` had **no connect or read timeout**, which would have made that permanent rather than transient; `application-test.yml` never pinned `app.resend.api-key`, so an exported `RESEND_API_KEY` made the suite send real email through the real account; and the new ADR's closing bullet described `dev` rather than the branch it was on. The review's *impact* claim on the first — that the visitor gets a 500 — proved false when measured: Spring 7.0.8 dispatches `AFTER_COMMIT` from `afterCompletion`, which swallows and logs. The fix stayed, its stated reason changed, and the test written to the predicted symptom had to be rebuilt after passing against the unfixed code. 235 tests to 253 to 260. See the 2026-09-03 ADR in `docs/DECISIONS.md` and the 2026-09-03 and 2026-09-04 entries in `AGENT_LOG.md`

- [x] **Not in the original plan** — password-reset link UX, issues #185 and #187. Reported from the live site: reusing a reset link let you fill the form, and the button then appeared to do nothing. Two defects, one cause. The backend reports a spent token as a field error keyed `token`; the reset form has no `token` input, since the token comes from the query string; and `errorInterceptor` deliberately suppresses its toast whenever field errors are present. So the rejection was suppressed as "will be shown inline" by a form that could not show it — the same shape as #92/#106, now the third occurrence. #187 adds the check that should have come first: `POST /auth/password-reset/validate` (a read, never a consume — merged) and a load-time validation that replaces the form with a dead-link state and a "Request a new link" action, staying on the page. A 429 or a 5xx from that check is deliberately **not** a dead link and fails open, since the limit is per requester IP and says nothing about the token. Backend landed on `feat/reset-token-validate`; frontend on `feat/reset-link-ux` (345 → 360 tests); both landed 2026-09-04 in PR #189. See `AGENT_LOG.md`

- [x] **Not in the original plan** — the About page becomes the landing page (owner request, 2026-09-24, #221; follow-up #224: About first in the nav, the owner's name as the home title, and a server-side 301 from `/about`). Promoted to `main` on 2026-09-24 (#226), after the About page was given its content that evening. `/` renders About, the project list moves to `/projects`, `/about` redirects to `/`, `/projects/:id` is unchanged. The projects feature is now mounted under a named `projects` prefix, which retires the `path: ''` prefix-match backtracking described under the performance pass above rather than working around it; About sits at `''` with `pathMatch: 'full'`. Nav, back links, sitemap, Vitest and E2E journeys updated; needs a visual check in a browser before merge.
