...
**Default branch fix (2026-08-02):** the repo's default branch was a stale `master` (57 commits behind `main`, just the original skeleton) from repo creation until now, even though every real PR (#76, #77, #79, #80) targeted `main`. GitHub only auto-links a PR's issues in its "Development" sidebar, and only auto-closes them on merge, when the PR's base is the *default* branch — so every one of those PRs' `Closes #N` references had silently done nothing, invisible unless you specifically checked `closingIssuesReferences` via the API. Fixed: default branch switched to `main`, `master` deleted. Separately, GitHub's documentation requires a keyword before *each* issue number, so always use one `Closes #N` per line. **Correction 2026-08-27:** this file previously said that had been *confirmed here*, by a comma-separated list linking only its first issue. It had not. PR #77's `Closes #20, #21, #22, #23, #69.` links **zero** issues, not one, and so does PR #76 — both are fully explained by the default-branch mechanism above, and neither is evidence about commas either way. The rule stands on GitHub's documentation; the local proof of it was a misattribution. see `CLAUDE.md`'s PR conventions and `AGENT_LOG.md` for the full writeup. **Superseded 2026-08-27:** the default branch is now `dev`, and for the same mechanism — `dev` became the integration branch, and a default of `main` would have voided every feature PR's keywords exactly as `master` did. `main` is production-only. ADR in `docs/DECISIONS.md`.
