...
> **Status (2026-09-24):** **both halves live; CI exists; both deploy pipelines are live on `main`, their rollback is proven, and they have shipped real releases (#218, 87 commits including a migration; #220) — any push or merge to `main` now deploys.** Open in this milestone: #38, #45 and #46 describe exactly what those releases exercised and can likely be closed (owner's call); #41 is deferred on purpose; #181 and #203 are smaller follow-ups. *(Earlier status, 2026-09-05: the deploy automation was written but not switched on.)* The backend answers on `https://tarka1939.bieda.it`, Netlify serves the promoted frontend against it, TLS verifies clean on both (#47), the SPA fallback is confirmed on the deployed artifact (#39), Postgres is verified by its own migrations (#43), and the admin password is set (#121). **CI now runs both suites plus an API-client-staleness check on every pull request** (#193) — and caught a racy test on its first day that passes locally and failed on the runner. Structured logging landed off-by-default (#48). As of 2026-09-05, what remained was six items: the two deploy pipelines were *written* (#38, #45, PR #196) and waited on credentials and a Netlify switch that only the owner could do; #46 was those credentials; #42 was independent; #41 was deferred on purpose. (As of 2026-09-24 the credentials and the switch are done and both pipelines have run; #42 landed; what is left is the first promotion through the pipelines, and #41, still deferred.) Order and acceptance criteria in `docs/CI_PLAN.md`, setup steps in `docs/DEPLOY_PIPELINE_SETUP.md`, decisions in `docs/DECISIONS.md` 2026-09-04. **"Phase 5 is nearly done" is still the wrong reading**: the manual half is finished, which is the half that tells you what to automate. **Update (2026-09-24):** the deploy workflow files are now on `main` too (PR #215, a cherry-pick of #196's commit merged with `[skip ci]`, so nothing deployed), which is what makes `docs/CI_PLAN.md` §8 step 2's dispatch-against-`main` possible. Later the same day: owner setup done, both pipelines dispatched green against `main` (the backend's `command=` restriction and host-key pin exercised for real), and the rollback proven against a build that compiles and refuses to start under `prod` — site back in ~2m17s, run red, failed jar kept. Next: the `dev` → `main` promotion, which is now the pipeline's first *change* rather than its first run.
>
