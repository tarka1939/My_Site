...
> **Status (2026-09-03):** **unpaused and in progress.** Host chosen: Mikrus, a NAT'd LXC container on Ubuntu 24.04. Done — user and firewall, Postgres 16 with the `mysite` role and database, the public subdomain `tarka1939.tojest.dev` with TLS terminated by the provider, and the jar built and copied to the host. Remaining — the JRE, the environment file, the systemd unit, the admin password (#121), and all of the Netlify half. Step-by-step in `docs/DEPLOYMENT.md`; the exposure decision and its rejected alternatives are an ADR in `docs/DECISIONS.md`, 2026-09-03.
>
