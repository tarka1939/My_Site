...
**Also surfaced during Phase 6, tracked separately** (none in the original checklist; all found while drafting real content or measuring real build output): #86 no description clamp on the list page, #87 image alt text hardcoded as "screenshot N", #88 no ordering field on `Project`, #89 API-origin `preconnect` (blocked on Phase 5), #90 frontend dependency audit. The first three are effectively prerequisites for #49 — merging real content without them would visibly degrade the site.
