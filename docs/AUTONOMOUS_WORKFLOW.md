...
Every dispatch **must specify a model**. Omitting it is not a neutral default — it silently inherits the Senior Dev's own model, which through Phase 6 meant every agent ran on Opus, including whitespace transforms and fix rounds applying a list of already-specified findings. `CLAUDE.md`'s "Choosing a model when dispatching" holds the table; `docs/AGENT_WORKFLOW.md` holds the cost evidence behind it.

Three obligations this places on the Senior Dev specifically, since they are the reason the cheaper rows are safe:

1. **Write the acceptance criteria before choosing the cheap row.** That is the test `CLAUDE.md` uses, and it is stricter than "is the contract settled" — the frontend agent's refusal to snap picked dates happened against a settled ADR, and was right because nobody had written "and do not silently rewrite stored rows" into the brief. If you cannot state what done looks like in advance, the judgement has not been done yet and delegating it cheaply just hides that. Choosing wrongly in the cheap direction is the live risk of this policy.
...
3. **Scope the brief.** Model and reasoning effort both come from the agent definition (`.claude/agents/*.md`), so those are set once rather than remembered per dispatch; brief scope is the part that is genuinely per-task. Name the sections that bear on the task rather than instructing a full read of documents that now run to thousands of lines, and scope a review to the diff's risk rather than a standing-doc sweep. The role definitions now carry this instruction themselves, so it holds whether or not a brief repeats it.
