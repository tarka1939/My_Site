...
4. **Every new colour that can carry two values is defined per scheme, with its computed ratio recorded**, following the pattern `--color-text-muted` and `--color-error` already set. No new token may be declared once and inherited by both grounds — that is precisely how #116 and the border defect happened, twice.
