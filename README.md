...
**Phase 6 (content & polish) is in progress** — a performance pass, a project date period, the content-rendering work real portfolio copy depends on, and a substantial hardening pass across both forms have landed. Two defects found by using the deployed site were fixed on 2026-09-04: a spent password-reset link rendered a working form and then silently swallowed the refusal, because the rejection is a field error keyed `token` and the form has no `token` input for it to land in (#185, #187). What remains needs decisions rather than code: per-image alt text, a contact route that does not depend on the contact endpoint, the canonical domain the sitemap needs, and a red error toast that fires alongside the reset page's own calmer notice (#191).
...
**What is not done is the entire automated half.** Every deploy so far has been by hand — `mvn package`, `scp`, `systemctl restart`. `.github/workflows/` holds a placeholder README and nothing else, so there is no CI on pull requests either: the test gates in this repository are run by whoever remembers to run them. Six issues remain open and none has been started — the frontend and backend deploy pipelines (#38, #45), the Dockerfile and compose file (#41, #42), moving secrets out of `/etc/mysite/env` into a CI secret store (#46), and structured logging (#48).
