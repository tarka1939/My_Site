...
**Phase 8 (polishing and cleaning) has started** with the visual design the site never had (#152) — a token layer where every colour is defined per scheme with its measured contrast ratio recorded beside it, a real type scale, self-hosted faces rather than the Google Fonts CDN, and a card grid that generates per-project artwork where no image exists. What remains in the milestone is security hardening deferred from Phases 1-3: a Content-Security-Policy (#122) and the JavaScript-readable admin JWT (#123).
