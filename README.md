...
**Phase 6 (content & polish) is in progress** — a performance pass, a project date period, the content-rendering work real portfolio copy depends on, and a substantial hardening pass across both forms have landed. Two defects found by using the deployed site were fixed on 2026-09-04: a spent password-reset link rendered a working form and then silently swallowed the refusal (#185, #187). September added the things a reader actually sees: project descriptions are Markdown with a live admin preview (#206), the card thumbnails and detail-page gallery were resized after measuring that every image on the site painted at under a fifth of its natural size (#205, #211), and there is an editable About page (#213), which is now the site's landing page at `/`, with the project list at `/projects`. What remains needs decisions rather than code: per-image alt text (#97), a contact route that does not depend on the contact endpoint (#118), project ordering (#88), and a red error toast that fires alongside the reset page's own calmer notice (#191). Two tests flake under load and are tracked rather than papered over (#203, #209), and no linter is configured, which leaves the Markdown XSS boundary guarded by comments alone (#210).
...
- **Live URLs:** frontend `https://krzysztof-tarka.netlify.app`, backend `https://tarka1939.bieda.it`. Both serve `main`, which is promoted from `dev` by pull request and deployed on merge. Five portfolio projects are published (checked 2026-09-24).
...
Worth being precise about what that does and doesn't demonstrate: the two sessions ran sequentially in the *same* worktree, so the separation was a prompt-level instruction, not a filesystem boundary — and `docs/AGENT_WORKFLOW.md` is explicit that those are not the same thing. The frontend's client is also *generated* from the contract, so its agreement is partly mechanical rather than independent. The genuinely isolated exercise has since run, in Phase 7a: an agent forbidden from reading `/backend` found that the admin list called the *public* endpoint, which filters to published projects, while the edit form called one that 404s for drafts — so the feature could not have worked. See `AGENT_LOG.md`, 2026-08-21.

**`dev` integrates, `main` ships.** Feature branches are cut from `dev` and merge back into it; only a promotion PR merges `dev` into `main`, which is what Netlify and the VPS deploy. `dev` is also GitHub's *default* branch, and that part is forced rather than chosen: GitHub links and closes a PR's issues only when the PR's base is the default branch, so leaving `main` as the default would silently void every feature PR's `Closes #N` — exactly what a stale `master` default already did to four PRs earlier in this project. See `docs/DECISIONS.md`, 2026-08-27.

**One task, one worktree, one branch, one session.** Concurrent sessions never share a working directory — each gets its own `git worktree`, which is what makes simultaneous commits safe. Two `PreToolUse` hooks cover the mechanically checkable parts: force-pushes, hard resets and direct checkouts of `main`/`master`/`dev` are denied unconditionally, and `Edit`/`Write` calls outside a session's assigned worktree are denied when that session opts in by exporting `CLAUDE_WORKTREE_ROOT`. The second is opt-in by design, so it doesn't restrict ordinary single-session work — which means it's a guard rail, not a guarantee.

That last paragraph was **false from 2026-08-07 to 2026-08-27** — the hooks were added in commit `7bd9b86` partway through Phase 4 and were inert from that moment until Phase 8, which is worth stating rather than quietly fixing. (An earlier draft of this correction said "the first eight phases"; the hooks did not exist for Phases 1-3, and overclaiming inside a correction is the same fault being corrected.) Both hooks parsed their input with `jq`, which is not installed on the machine this repo is developed on — so both fell through to "allow" for every command they were written to deny, while this file called them unconditional. They were rewritten on 2026-08-27 to parse with Python, to deny rather than permit when they cannot read their input, and to compare real filesystem paths instead of string prefixes. Neither had a test, and nothing had ever tried a command they were supposed to block. `docs/AGENT_WORKFLOW.md` carries the full account; the transferable part is that a guard which fails open is worse than no guard, because the documentation describing it becomes the thing people trust.
